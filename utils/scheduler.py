...
METHOD_HABITS_TEMPLATE = """
You're a productivity coach. Suggest 3 habits someone could build using the {method_name} method.
Only list the habit names in a comma-separated list.
""".strip()
...
def create_schedule(habits_df: pd.DataFrame, day_of_week: DayOfWeek) -> List[ToDo]:
    chat_gpt = ChatGoogleGenerativeAI(model="gemini-2.0-flash", temperature=0)
    prompt = PromptTemplate(template=TODAY_HABITS_TEMPLATE, input_variables=["habits_table", "day_of_week"])
    ...
    prompt = PromptTemplate(template=SCHEDULE_HABITS_TEMPLATE, input_variables=["habits_table"])
    habits_table = scheduled_df[["name", "preferred_time", "duration"]].to_markdown(index=None)
    chain = LLMChain(llm=chat_gpt, prompt=prompt)
...
def update_schedule(schedule: List[ToDo], question: str) -> List[ToDo]:
    chat_gpt = ChatGoogleGenerativeAI(model="gemini-2.0-flash", temperature=0)
    prompt = PromptTemplate(template=SCHEDULE_QUESTION_TEMPLATE, input_variables=["schedule", "question"])
    schedule_text = "\n".join([str(todo) for todo in schedule])
    chain = LLMChain(llm=chat_gpt, prompt=prompt)
    result = chain.run(schedule=schedule_text, question=question)
    return parse_todo_items(result)

def suggest_habits_based_on_method(method_name: str) -> List[str]:
    chat_gpt = ChatGoogleGenerativeAI(model="gemini-2.0-pro", temperature=0.5)
    prompt = PromptTemplate(input_variables=["method_name"], template=METHOD_HABITS_TEMPLATE)
    chain = LLMChain(llm=chat_gpt, prompt=prompt)
    result = chain.run(method_name=method_name)
    return [habit.strip() for habit in result.split(",") if habit.strip()]
